<?php
require_once __DIR__.'/../includes/bootstrap.php';require_auth();$u=current_user($pdo);$uid=(int)$u['id'];$teams=user_teams($pdo,$uid);$teamId=current_team_id($pdo,$uid);$members=[];if($teamId){$ms=$pdo->prepare("SELECT u.id,u.name FROM team_members tm JOIN users u ON u.id=tm.user_id WHERE tm.team_id=? ORDER BY u.name");$ms->execute([$teamId]);$members=$ms->fetchAll();}$id=filter_input(INPUT_GET,'id',FILTER_VALIDATE_INT);
$stmt=$pdo->prepare("SELECT * FROM tasks WHERE id=? AND user_id=?");$stmt->execute([$id,$uid]);$task=$stmt->fetch();if(!$task){flash('error','Task not found.');redirect('/tasks/index.php');}
$editing=isset($task);
...
?>
<?php $pageTitle='Edit Task';$activePage='tasks';require __DIR__.'/../includes/header.php';?>
<section class="form-layout"><div class="form-copy"><p class="eyebrow">UPDATE TASK</p><h2>Keep every detail accurate.</h2><p>Adjust status, deadline and progress as work evolves.</p></div><form class="panel task-form" method="post"><input type="hidden" name="csrf_token" value="<?=e(csrf_token())?>"><?php if($errors):?><div class="alert alert-error full"><?=implode('<br>',array_map('e',$errors))?></div><?php endif;?>
<label class="full">Title<input name="title" value="<?=e($data['title'])?>" required></label><label class="full">Description<textarea name="description" rows="6"><?=e($data['description'])?></textarea></label><label>Status<select name="status"><option value="todo" <?=$data['status']==='todo'?'selected':''?>>To Do</option><option value="in_progress" <?=$data['status']==='in_progress'?'selected':''?>>In Progress</option><option value="completed" <?=$data['status']==='completed'?'selected':''?>>Completed</option></select></label><label>Priority<select name="priority"><option value="low" <?=$data['priority']==='low'?'selected':''?>>Low</option><option value="medium" <?=$data['priority']==='medium'?'selected':''?>>Medium</option><option value="high" <?=$data['priority']==='high'?'selected':''?>>High</option></select></label><label>Category<input name="category" value="<?=e($data['category'])?>"></label><label>Team<select name="team_id"><option value="">Personal</option><?php foreach($teams as $team):?><option value="<?=$team['id']?>" <?=((int)($data['team_id']??0)===(int)$team['id'])?'selected':''?>><?=e($team['name'])?></option><?php endforeach;?></select></label><label>Assign to<select name="assigned_to"><option value="">Unassigned</option><?php foreach($members as $member):?><option value="<?=$member['id']?>" <?=((int)($data['assigned_to']??0)===(int)$member['id'])?'selected':''?>><?=e($member['name'])?></option><?php endforeach;?></select></label><label>Tags<input name="tags" value="<?=e($data['tags']??'')?>" placeholder="frontend, api, urgent"></label><label>Due date<input type="date" name="due_date" value="<?=e($data['due_date'])?>"></label><label class="full">Progress<input type="range" name="progress" min="0" max="100" value="<?=e((string)$data['progress'])?>" oninput="this.nextElementSibling.value=this.value+'%'"><output><?=e((string)$data['progress'])?>%</output></label><div class="form-actions full"><a class="btn btn-ghost" href="tasks/index.php">Cancel</a><button class="btn btn-primary">Save Changes</button></div></form></section>
